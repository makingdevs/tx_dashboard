defmodule TxDashboardWeb.TransactionLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all transactions", %{conn: conn, transaction: transaction} do
      {:ok, _index_live, html} = live(conn, Routes.movements_path(conn, :index))
    ...
    test "saves new transaction", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, Routes.movements_path(conn, :index))

      assert index_live |> element("a", "New Transaction") |> render_click() =~
               "New Transaction"

      assert_patch(index_live, Routes.movements_path(conn, :new))
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, Routes.movements_path(conn, :index))
    ...
    test "updates transaction in listing", %{conn: conn, transaction: transaction} do
      ...
      assert_patch(index_live, Routes.movements_path(conn, :edit, transaction))
  ...
  describe "Show" do
    ...
    test "displays transaction", %{conn: conn, transaction: transaction} do
      {:ok, _show_live, html} = live(conn, Routes.details_path(conn, :show, transaction))
    ...
    test "updates transaction within modal", %{conn: conn, transaction: transaction} do
      {:ok, show_live, _html} = live(conn, Routes.details_path(conn, :show, transaction))

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Transaction"

      assert_patch(show_live, Routes.details_path(conn, :edit, transaction))
      ...
      {:ok, _, html} =
        ...
        |> form("#transaction-form", transaction: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, Routes.details_path(conn, :show, transaction))
